...
static inline float dist_sq_sse(const float* __restrict__ a, const float* __restrict__ b) {
    ...
    __m128 shuf = _mm_shuffle_ps(sum, sum, _MM_SHUFFLE(2, 3, 0, 1)); sum = _mm_add_ps(sum, shuf);
    shuf = _mm_shuffle_ps(sum, sum, _MM_SHUFFLE(1, 0, 3, 2)); sum = _mm_add_ps(sum, shuf);
...
static double get_minutes_since(const char* js) {
    ...
    int y1,m1,d1,h1,mi1,s1,y2,m2,d2,h2,mi2,s2; parse_ts(ra, y1,m1,d1,h1,mi1,s1); parse_ts(ts, y2,m2,d2,h2,mi2,s2);
...
static void vectorize(const char* js, float vec[DIMS]) {
    float amount = get_num(js, "\"amount\""); vec[0] = clamp01(amount / MAX_AMOUNT);
    vec[1] = clamp01(get_num(js, "\"installments\"") / MAX_INST);
    float avg_amount = get_num(js, "\"avg_amount\""); vec[2] = avg_amount>0 ? clamp01((amount/avg_amount)/AVG_RATIO) : 0.0f;
    ...
    vec[7] = clamp01(get_num(js, "\"km_from_home\"") / MAX_KM);
    vec[8] = clamp01(get_num(js, "\"tx_count_24h\"") / MAX_TX24);
    vec[9] = get_bool(js, "\"is_online\"") ? 1.0f : 0.0f;
    vec[10] = get_bool(js, "\"card_present\"") ? 1.0f : 0.0f;
    vec[11] = is_known_merchant(js) ? 0.0f : 1.0f;
    char mcc[16]; int mcc_len = get_mcc(js, mcc); vec[12] = mcc_risk(mcc, mcc_len);
    float merch_avg = 0; const char* merch = find_key(js, "\"merchant\"");
    if (merch) { const char* ma = find_key(merch, "\"avg_amount\""); if (ma) { ma+=12; while(*ma&&(*ma=='"'||*ma==':'||*ma==' '))ma++; merch_avg=strtof(ma,nullptr); } }
...
static float knn_search(const float query[DIMS]) {
    ...
    for (int c = 0; c < g_ncent; c++) {
        ...
        if (n_probe < NPROBE) {
            probe[n_probe++] = {d, c};
            if (n_probe == NPROBE) for (int j = NPROBE/2-1; j >= 0; j--) {
                int i = j; while (2*i+1<NPROBE) {
            ...
            probe[0] = {d, c}; int i = 0; while (2*i+1<NPROBE) {
                int ch=2*i+1; if(ch+1<NPROBE && probe[ch+1].d>probe[ch].d) ch++;
                if(probe[i].d>=probe[ch].d) break;
                CDist t=probe[i]; probe[i]=probe[ch]; probe[ch]=t; i=ch;
            }
    ...
    for (int p = 0; p < n_probe; p++) scan_range(query, g_offsets[probe[p].idx], g_offsets[probe[p].idx + 1], top, n_top);
    if ((n_top == KNN ? top[0].dist : 1e30f) > 1.0f) scan_range(query, 0, g_nrefs, top, n_top);
...
static int process_request(const char* req, int reqlen, char* resp) {
    ...
    if (memcmp(req, "POST", 4) == 0 && strstr(req, "/fraud-score")) {
        ...
        float query[DIMS]; vectorize(body, query); float score = knn_search(query);
        char json[128]; int jlen = snprintf(json, sizeof(json), "{\"approved\":%s,\"fraud_score\":%.1f}", score < THRESHOLD ? "true" : "false", score);
        return snprintf(resp, BUF_SIZE, "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: %d\r\nConnection: close\r\n\r\n%s", jlen, json);
    }
    return snprintf(resp, BUF_SIZE, "HTTP/1.1 404 Not Found\r\nContent-Length: 0\r\nConnection: close\r\n\r\n");
}

struct ConnBuf { char buf[BUF_SIZE]; int len; }; static ConnBuf g_conns[65536];
...
int main() {
    ...
    listen(sfd, SOMAXCONN); fcntl(sfd, F_SETFL, fcntl(sfd, F_GETFL, 0) | O_NONBLOCK);
    int epfd = epoll_create1(0); struct epoll_event ev; ev.events = EPOLLIN; ev.data.fd = sfd; epoll_ctl(epfd, EPOLL_CTL_ADD, sfd, &ev);
    struct epoll_event events[MAX_EVENTS]; char resp[BUF_SIZE];
    while (true) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (fd == sfd) {
                while (true) {
                    ...
                    g_conns[cfd].len = 0; ev.events = EPOLLIN | EPOLLET; ev.data.fd = cfd; epoll_ctl(epfd, EPOLL_CTL_ADD, cfd, &ev);
                }
            } else {
                ConnBuf& cb = g_conns[fd]; bool closed = false;
                while (true) {
                ...
                if (!hdr_end) { if (cb.len >= BUF_SIZE-1) { epoll_ctl(epfd,EPOLL_CTL_DEL,fd,nullptr); close(fd); } continue; }
                int rlen = process_request(cb.buf, cb.len, resp);
                if (rlen > 0) { int sent = 0; while (sent < rlen) { int w = write(fd, resp+sent, rlen-sent); if (w <= 0) break; sent += w; } }
